...
std::string ToTzStringImpl(const WinTimeZoneRegistryEntry& entry) {
  const auto std_offset = 60 * (entry.bias + entry.standard_bias);
  ...
  const auto dst_offset = 60 * (entry.bias + entry.daylight_bias);
  ...
  const auto last_daylight_date = AdjustWinSystemTime(entry.daylight_date);
  ...
  const auto last_standard_date = AdjustWinSystemTime(entry.standard_date);
