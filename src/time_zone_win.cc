...
struct TransitionTable {
  ...
  const std::string proleptic_tz_string;
};
...
class TransitionTableBuilder {
 ...
 private:
  ...
  void Initialize(const WinTimeZoneRegistryInfo& info) {
    ...
    // Main loop — always runs, regardless of whether the first entry is fixed.
...
class WinZoneInfoSource : public ZoneInfoSource {
 public:
  ...
  explicit WinZoneInfoSource(std::vector<char>&& data) : data_(std::move(data)) {}
