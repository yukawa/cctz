...
// Transitions extracted from Win32TimeZoneRegistryEntry (==REG_TZI_FORMAT) for
// the target year. Each Win32TimeZoneRegistryEntry can provide up to three
// transitions in a year.
// The most tricky part is that Win32TimeZoneRegistryEntry gives us localtime in
...
// offset.
// See ResolveSystemTime() on how Win32TimeZoneRegistryEntry is interpreted.
...
class AbbreviationMapBuilder {
 public:
  ...
  void Add(const Win32TimeZoneRegistryEntry& info) {
...
class TimeZoneRegistry {
 public:
  ...
  const std::uint_fast32_t FirstYear() const {
    if (timezone_info_.entries.size() < 2) {
      return 0;
    }
    return timezone_info_.first_year;
  }
  const std::uint_fast32_t LastYear() const {
    if (timezone_info_.entries.size() < 2) {
      return 0;
    }
 ...
 private:
  ...
  static bool IsFixedTimeZone(const Win32TimeZoneRegistryEntry& entry) {
    return entry.standard_date.month == 0 && entry.daylight_date.month == 0;
  }

  static bool ResolveSystemTime(const Win32SystemTime& system_time, year_t year,
                                civil_second* result) {
  ...
  static std::deque<RawTransitionInfo> ParseTimeZoneInfo(
      const Win32TimeZoneRegistryEntry& format, year_t year) {
