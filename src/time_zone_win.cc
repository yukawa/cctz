 ...
 private:
  ...
  void Initialize(const WinTimeZoneRegistryInfo& info) {
    ...
    const year_t first_year =
        info.entries.size() == 1
            ? 1970
            : std::min<cctz::year_t>(info.first_year, 1970);
    ...
    prolepic_tz_string_ = ToTzString(info);
  }
  ...
  void AddTransition(const TransitionInfo& transition_info) {
    ...
    const std::string abbr =
        "GMT" + cctz::FixedOffsetToAbbr(cctz::seconds(transition_info.offset));
  ...
  static bool EndsWithFixed(const WinTimeZoneRegistryInfo& info) {
    return info.entries.empty() ? false : IsFixedTimeZone(info.entries.back());
  }

  static WinSystemTime AdjustWinSystemTime(const WinSystemTime& system_time) {
    // Special rule for "23:59:59.999".
    // https://stackoverflow.com/a/47106207
    if (system_time.hour == 23 && system_time.minute == 59 &&
      ...
      if (new_day_of_week > system_time.day_of_week) {
        ...
        return WinSystemTime(system_time.year, system_time.month,
                             new_day_of_week, new_day, 0, 0, 0, 0);
      }
  ...
  static std::string ToTzString(const WinTimeZoneRegistryInfo& info) {
    if (info.entries.empty()) {
      return "";
    }
    const auto last_entry = info.entries.back();
    const auto std_offset = 60 * (last_entry.bias + last_entry.standard_bias);
    const auto std_offset_min = std::abs((std_offset % 3600) / 60);
    ...
    const std::string dst_tz =
        dst_offset == 0
            ...
            : "<GMT" + cctz::FixedOffsetToAbbr(cctz::seconds(-dst_offset)) +
                  ...
                  (dst_offset_min == 0 ? ""
                                       : ":" + std::to_string(dst_offset_min));
    const auto last_daylight_date =
        AdjustWinSystemTime(last_entry.daylight_date);
    std::string daylight_date_min = std::to_string(last_daylight_date.minute);
    if (daylight_date_min.size() == 1) {
      daylight_date_min = "0" + daylight_date_min;
    }
    std::string daylight_date_sec = std::to_string(last_daylight_date.second);
    if (daylight_date_sec.size() == 1) {
      daylight_date_sec = "0" + daylight_date_sec ;
    }
    const std::string dst_start =
        last_daylight_date.month == 0
            ...
            : ",M" + std::to_string(last_daylight_date.month) + '.' +
                  ...
                  std::to_string(last_daylight_date.hour) + ":" +
                  daylight_date_min + ":" + daylight_date_sec;
    const auto last_standard_date =
        AdjustWinSystemTime(last_entry.standard_date);
    std::string standard_date_min = std::to_string(last_standard_date.minute);
    if (standard_date_min.size() == 1) {
      standard_date_min = "0" + standard_date_min;
    }
    std::string standard_date_sec = std::to_string(last_standard_date.second);
    if (standard_date_sec.size() == 1) {
      standard_date_sec = "0" + standard_date_sec;
    }
    const std::string std_start =
        last_standard_date.month == 0
            ...
            : ",M" + std::to_string(last_standard_date.month) + '.' +
                  ...
                  std::to_string(last_standard_date.hour) + ":" +
                  standard_date_min + ":" + standard_date_sec;
