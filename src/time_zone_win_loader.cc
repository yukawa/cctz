...
std::wstring Utf8ToUtf16(const std::string& utf8str) {
  ...
  // Fast-path for small strings.
  {
    ...
    if (num_chars > 0 && num_chars <= buffer_size) {
      return std::wstring(buffer, num_chars);
    }
  }

  // Slow-path: query required size, then convert.
  const int required =
      ::MultiByteToWideChar(CP_UTF8, MB_ERR_INVALID_CHARS, utf8str_ptr,
                            utf8str_len, nullptr, 0);
  if (required <= 0) {
    return std::wstring();
  }
  auto wstr = std::unique_ptr<wchar_t[]>(new wchar_t[required]);
  const int written =
      ::MultiByteToWideChar(CP_UTF8, MB_ERR_INVALID_CHARS, utf8str_ptr,
                            utf8str_len, wstr.get(), required);
  if (required != written) {
    return std::wstring();
  }
  return std::wstring(wstr.get(), required);
}
